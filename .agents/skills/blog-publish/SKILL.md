...
- `../blog-shared/scripts/text_length.py` (deterministic stage validation)
...
2. Read frontmatter `title`; set `updated` to today `YYYY-MM-DD` via targeted `Edit`; ensure `permalink: ""` stays manual (or author-defined), `lang: vi`, and flip `publish` from `false` to `true`. Run `python3 .agents/skills/blog-shared/scripts/text_length.py --post <post> --stage publish` to verify.
